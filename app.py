...
# Import services
try:
    from services.background_agent import BackgroundAgent
...
# Nutrition database with categories
NUTRITION_DB = {
...
def analyze():
    ...
    try:
        ...
        if BACKGROUND_AGENT_ENABLED:
            job_id = background_agent.submit_job('food_analysis', {'filepath': filepath})
            return jsonify({
                "status": "processing",
                "job_id": job_id,
            ...
            result = process_food_analysis({'filepath': filepath}, lambda p, m: None)
